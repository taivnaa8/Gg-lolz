...
function sendResponse(dataObject) {
  ...
  .then(response => {
    if (response.ok) {
      console.log('Хариу амжилттай илгээгдлээ!');
    }
  })
...
document.getElementById('btn-yes').addEventListener('click', () => {
  ...
  // 1. Sends email when she clicks YES!
  sendResponse({ status: "Said YES to main proposal! ❤️" });
});

document.getElementById('btn-no').addEventListener('click', () => {
  noCount++;
  
  // 2. Sends email when she tries to click NO
  sendResponse({ status: `Clicked NO (Attempt #${noCount}) 😭` });
...
document.getElementById('place-own-confirm').addEventListener('click', () => {
  ...
  if (!answer){ alert('Бичээд батална уу гүнжтэн минь!🙂'); return; }
  chosenPlace = answer;
  finishFlow();
});
...
function finishFlow(){
  ...
  // 3. Sends full summary email with chosen Date & Place!
  sendResponse({
    status: "FINAL DATE PLAN CONFIRMED! 🎉",
